...
class Mage_Adminhtml_Block_Sales_Order_Creditmemo_Create_Items extends Mage_Adminhtml_Block_Sales_Items_Abstract
{
    ...
    protected function _prepareLayout()
    {
        ...
        // CYBERSOURCE PAYMENT METHOD RETURNS TRUE - ALLOWS REFUND - BUT IT DOESN'T SUPPORT ORDERS OLDER THAN 60 DAYS FOR API INITIATED REFUND
        if ($this->getCreditmemo()->canRefund())
        {
            $onlineEnabled = false;

            if ($this->getCreditmemo()->getInvoice() && $this->getCreditmemo()->getInvoice()->getTransactionId()
                && $this->getOrder()->getCreatedAtDate() > Zend_Date::now()->subDay(60))
            {
                $this->setChild(
                    'submit_button',
                    $this->getLayout()->createBlock('adminhtml/widget_button')->setData(array(
            ...
            // ONLY SHOW OFFLINE REFUND IF NO ONLINE AVAILABLE
            if(!$onlineEnabled)
            {
                $this->setChild(
                    'submit_offline',
                    $this->getLayout()->createBlock('adminhtml/widget_button')->setData(array(
                        'label'     => Mage::helper('sales')->__('Refund Offline - Order older than 60days - Process manually'),
                        'class'     => 'save submit-button',
