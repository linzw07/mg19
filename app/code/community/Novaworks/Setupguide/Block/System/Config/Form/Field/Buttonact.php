<?php
class Novaworks_Setupguide_Block_System_Config_Form_Field_Buttonact extends Mage_Adminhtml_Block_System_Config_Form_Field
{
    ...
    protected function _getElementHtml(Varien_Data_Form_Element_Abstract $element)
    {
        ...
        if ($values = $element->getValues()) {
            foreach ($values as $option) {
            	$key = Mage::getSingleton('adminhtml/url')->getSecretKey($option['value']);
                $html.= "<button type=\"button\" class=\"scalable\" onclick=\"setLocation('".Mage::helper("adminhtml")->getUrl($option['value'])."')\" style=\"\"><span><span><span>".$option['label']."</span></span></span></button>";
            }
        }
	    $html.= '</div>';
