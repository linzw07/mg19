		...
		if($params['isAjax'] == 1){
			...
			try {
				...
				if (!$cart->getQuote()->getHasError()){
					$message = $this->__('The product <span>%s</span> was added to your shopping cart.<br/> <a href="%s" class="button-readmore">%s</a>', Mage::helper('core')->htmlEscape($product->getName()),Mage::helper('checkout/cart')->getCartUrl(),$this->__('View Cart'));
	...
	public function compareAction(){
		...
		if ($productId = (int) $this->getRequest()->getParam('product')) {
			...
			if ($product->getId()/* && !$product->isSuper()*/) {
				...
				$response['status'] = 'SUCCESS';
				$response['message'] = $this->__('The product <span>%s</span> has been added to comparison list.', Mage::helper('core')->escapeHtml($product->getName()));
				Mage::register('referrer_url', $this->_getRefererUrl());
		...
		if ($productId = (int) $this->getRequest()->getParam('product')) {
			...
			if($product->getId()) { 
				...
				if($item->getId()) {
					$item->delete();
					$response['status'] = 'SUCCESS';
					$response['message'] = $this->__('The product <span>%s</span> has been removed from comparison list.', $product->getName()); 
				...
				if (!$productId) {
					...
					if (!$product->getId() || !$product->isVisibleInCatalog()) {
						...
						try {
							...
							$message = $this->__('The product <span>%1$s</span> has been added to your wishlist.', $product->getName());
							$response['status'] = 'SUCCESS';
							$response['message'] = $message; 
